...
function CreateRole({ onClose, onCreated }) {
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        // Normaliza el nombre del input eliminando acentos y espacios
        const normalizedInputName = removeAccentsAndSpaces(
            values.Name_Role
        );

        // Normaliza los nombres existentes en la lista de roles
        ...
        // Prepara los datos para enviar, conservando la inicial en mayúscula y eliminando espacios
        const dataToSend = {
            ...values,
            Name_Role: normalizedInputName.replace(/\s+/g, ' ').replace(/^(.)/, (match) => match.toUpperCase())
        };

        // Crea el rol
        createRole(dataToSend);

        // Ejecuta funciones adicionales al crear el rol
        onCreated();
    ...
    const onCancel = () => {
        // Cierra la ventana modal sin hacer cambios
        onClose();
    };
                                ...
                                <div className="buttonconfirm">
                                    <div className="mb-3">
                                        <button
                                            ...
                                            type="submit"
                                            title='Se guarda la información recién ingresada en el sistema.'
                                        >
                                            Confirmar
                                        </button>
                                        <button
                                            ...
                                            onClick={onCancel}
                                            type="button"
                                            title='Se cancela la información recién ingresada en el sistema.'
                                        >
                                            Cancelar
                                        </button>
                                    </div>
