...
function WaiterPage() {
    ...
    // Función para manejar el cambio de estado de los meseros.
    const handleToggleUserStatus = async (id) => {
        try {
            // Realizar la acción de cambio de estado y obtener el resultado.
            const { hasError } = await toggleUserStatus(id);

            // Si hay un error, retornar y manejar según sea necesario.
            if (hasError) return;

            // Actualizar el estado local para reflejar el cambio.
            setAllWaiter((prevUsers) =>
                prevUsers.map((user) =>
                    user.ID_User === id ? { ...user, State: !user.State } : user
                )
            );
        } catch (error) {
            console.error(error);
                                        ...
                                        <div className="col-md-6">
                                            <div className="form-group">
                                                <input
                                                    ...
                                                    title="Buscar uno o varios meseros por algún dato de los meseros."
                                                />
                                            </div>
                                        </div>
                                                ...
                                                <tbody>
                                                    {visibleWaiters.map((waiter) => (
                                                        <tr key={waiter.ID_User}>
                                                            ...
                                                            <td title="Número de identificación del mesero.">{waiter.Document}</td>
                                                            ...
                                                            <td>
                                                                <div style={{ display: "flex", alignItems: "center", padding: '3px' }}>
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        onClick={() => handleToggleUserStatus(waiter.ID_User)}
                                                                    >
                                                                        {waiter.State ? (
